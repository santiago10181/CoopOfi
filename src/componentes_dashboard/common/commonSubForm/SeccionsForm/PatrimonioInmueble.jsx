...
import DataPatrimonoInmuebles from "../dataSeccion/DataPatrimonoInmuebles"

const PatrimonoInmuebles = () => {
  return (
    <div className="border border-slate-200 p-6 rounded-lg grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
      <TitleSubForm
        Value="Patrimonio Inmuebles"
        className="text-xl font-semibold text-slate-800 md:col-span-2 lg:col-span-3"
      />
      
      {DataPatrimonoInmuebles.fields.map((item, index) => {
        // Usamos item.NameInputNum como 'name' único para el form
        const name = item.NameInputNum;

        if (item.isSelect) {
          const options =
            item.NameInputNum === "Tipo de Garantía"
              ? DataPatrimonoInmuebles.optionsTipoGarantia
              : DataPatrimonoInmuebles.optionsLineaCredito;
...
export default PatrimonoInmuebles;
