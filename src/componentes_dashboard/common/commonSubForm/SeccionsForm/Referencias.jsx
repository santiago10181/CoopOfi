...
import DataReferencias from "../dataSeccion/DataReferencias";
...
const Referencias = () => {
  return (
    <div className="border border-slate-200 p-6 rounded-lg grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
      ...
      {DataReferencias.fields.map((item, index) => {
        // Usamos item.NameInputNum como 'name' único para el form
        const name = item.NameInputNum;

        if (item.isSelect) {
          const options =
            item.NameInputNum === "Parentesco (Referencia diferente al cónyuge)" 
              ? DataReferencias.optionsParentesco
              : DataReferencias.optionsParentesco
