...
import DataInfoFinanciera from "../dataSeccion/DataInfoFinanciera";
...
const InfoFinanciera = () => {
  return (
    <div className="border border-slate-200 p-6 rounded-lg grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
      ...
      {DataInfoFinanciera.fields.map((item, index) => {
        ...
        if (item.isSelect) {
          const options =
            item.NameInputNum === "Tipo de Garantía"
              ? DataInfoFinanciera.optionsTipoGarantia
              : DataInfoFinanciera.optionsLineaCredito;
